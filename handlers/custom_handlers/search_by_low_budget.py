...
async def search(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(SearchState.low_budget)
    await state.update_data(search_by=search_types["low_budget"]["name"])
...
@router.message(SearchState.low_budget)
async def get_budget(message: Message, state: FSMContext) -> None:
    ...
    if not is_match is None:
        await state.update_data(low_budget=message.text)
